...
- D) Content Styling System

---

**Q2.** Which method of adding CSS is the professional best practice?
...
**Q4.** Which selector has the HIGHEST specificity?

- A) Element selector (`p`)
- B) Class selector (`.intro`)
- C) ID selector (`#hero`) ✅
- D) Universal selector (`*`)

---

**Q5.** What does `#ff0000` represent in HEX color?
...
**Q6.** What does the `a` in `rgba(0, 0, 0, 0.5)` control?
...
**Q7.** Which CSS property removes the underline from links?

- A) `text-style: none;`
- B) `text-decoration: none;` ✅
- C) `text-underline: false;`
- D) `font-decoration: none;`

---

**Q8.** What unit is relative to the ROOT element's font size?
...
**Q9.** What does `font-weight: 700;` produce?
...
**Q10.** Which tag links an external CSS file to HTML?
...
**Q1. Explain the three ways to add CSS to HTML. Which one is recommended for professional projects and why?**

> 1. **Inline CSS:** Written directly on HTML elements using the `style` attribute. Example: `<h1 style="color: blue;">Hello</h1>`. Not recommended because it mixes structure and style.
>
> 2. **Internal CSS:** Written inside a `<style>` tag in the `<head>` section. Styles only apply to that single page. Good for learning but not scalable.
>
> 3. **External CSS:** Written in a separate `.css` file and linked using `<link rel="stylesheet" href="style.css">`. This is the **recommended approach** because it fully separates HTML and CSS, allows one CSS file to style multiple pages, is cached by browsers for faster loading, and is maintainable and scalable for large projects.

---

**Q2. What is CSS specificity? Arrange these selectors from lowest to highest specificity: `#hero`, `p`, `.intro`, `*`.**

> CSS specificity determines which style rule wins when multiple rules target the same element. Each selector type has a weight:
>
> From **lowest** to **highest** specificity:
> 1. `*` (Universal) — Specificity: **0**
> 2. `p` (Element) — Specificity: **1**
> 3. `.intro` (Class) — Specificity: **10**
> 4. `#hero` (ID) — Specificity: **100**
>
> If a `<p>` element has `id="hero"` and `class="intro"`, and all three selectors set different colors, the **ID selector wins** because it has the highest specificity.

---

**Q3. Explain the difference between HEX, RGB, and RGBA color formats with one example of each.**

> - **HEX:** A 6-digit code starting with `#`. Each pair represents Red, Green, Blue (00-FF).
>   ```css
>   color: #3498db;    /* A medium blue */
>   ```
>
> - **RGB:** Uses the `rgb()` function with values 0-255 for Red, Green, Blue.
>   ```css
>   color: rgb(52, 152, 219);    /* Same medium blue */
>   ```
>
> - **RGBA:** Same as RGB but adds an alpha channel (0-1) for transparency.
>   ```css
>   color: rgba(52, 152, 219, 0.5);    /* Same blue at 50% opacity */
>   ```
>
> HEX and RGB produce the same solid colors. RGBA adds transparency control, which is useful for overlays and subtle effects.

---

**Q4. What is a font stack? Write a proper font stack for a modern website and explain how it works.**

> A font stack is a **list of fonts** specified in the `font-family` property, separated by commas. The browser tries each font in order and uses the first one it finds installed.
>
> ```css
> body {
...
>
> How it works:
> 1. Browser looks for **Poppins** (Google Font) — if loaded, uses it
> 2. If not found, tries **Segoe UI** (common on Windows)
> 3. If not found, tries **Arial** (available on almost all systems)
> 4. If nothing found, uses any **sans-serif** font available on the system
>
> The last item should always be a generic family (`serif`, `sans-serif`, `monospace`) as a guaranteed fallback.

---

**Q5. Write CSS that styles a page with: dark header (white text, centered), body text in a custom font at 16px with 1.6 line-height, and links that are blue with no underline but show underline on hover.**
