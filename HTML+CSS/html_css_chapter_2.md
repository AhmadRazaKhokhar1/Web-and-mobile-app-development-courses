...
**Q1.** What are the four layers of the CSS Box Model (from inside to outside)?
...
- A) Removes all borders from elements
- B) Adds a box shadow
- C) Includes padding and border within the specified width/height ✅
- D) Makes elements circular

---

**Q3.** How do you center a block element horizontally?

- A) `text-align: center;`
- B) `margin: auto 0;`
- C) `margin: 0 auto;` (with defined width) ✅
- D) `display: center;`

---

**Q4.** Which display value allows an element to sit inline but also accept width/height?
...
**Q5.** What happens when two vertical margins of 30px and 20px touch?

- A) Total gap is 50px
- B) Total gap is 30px (larger margin wins) ✅
- C) Total gap is 20px
- D) No gap appears

---

**Q6.** What does `overflow: hidden` do?

- A) Shows scrollbars
- B) Makes content transparent
- C) Clips content that exceeds the container ✅
- D) Increases the container size

---

**Q7.** Which property controls the space INSIDE the border?
...
**Q8.** What does `background-size: cover` do?

- A) Shrinks the image to fit inside without cropping
- B) Fills the entire container, may crop edges ✅
- C) Tiles the image repeatedly
- D) Stretches the image without maintaining aspect ratio

---

**Q9.** What is the shorthand order for `margin: 10px 20px 30px 40px`?
...
- A) They are the same
- B) `display: none` removes the element from layout; `visibility: hidden` hides it but keeps the space ✅
- C) `visibility: hidden` removes the element; `display: none` keeps the space
...
**Q1. Explain the CSS Box Model with all four layers. Use a real-world analogy.**

> The CSS Box Model consists of four layers from inside to outside:
>
> 1. **Content** — The actual text, image, or child elements (like a gift inside a box)
> 2. **Padding** — Space between the content and the border (like bubble wrap around the gift)
> 3. **Border** — The visible edge/outline of the element (like the cardboard box itself)
> 4. **Margin** — Space outside the border separating this element from others (like the space between packages on a shelf)
>
> Every HTML element follows this model. Understanding it is essential for controlling spacing and sizing in CSS.

---

**Q2. What is margin collapse? Give an example and explain how to avoid unexpected spacing.**

> Margin collapse occurs when two **vertical margins** (top and bottom) of adjacent elements **touch** — instead of adding together, only the **larger margin** is used.
>
> Example:
> ```css
> .box-a { margin-bottom: 40px; }
> .box-b { margin-top: 25px; }
> /* Expected gap: 40 + 25 = 65px */
> /* Actual gap: 40px (larger wins) */
> ```
>
> This only happens with **vertical** margins. Horizontal margins never collapse.
>
> To avoid unexpected spacing:
> - Use padding instead of margin where possible
> - Use only `margin-bottom` OR only `margin-top` consistently (not both)
> - Flexbox and Grid containers prevent margin collapse between children

---

**Q3. Explain the difference between `display: block`, `display: inline`, and `display: inline-block`. When would you use each?**

> - **`display: block`** — Element takes full width, starts on a new line, and accepts width/height. Use for sections, containers, and structural elements.
>
> - **`display: inline`** — Element takes only content width, sits on the same line, and does NOT accept width/height. Use for text-level elements like links and spans.
>
> - **`display: inline-block`** — Element sits on the same line like inline BUT accepts width/height like block. Use for navigation links, buttons, cards placed side by side, and stat boxes.
>
> ```css
> nav a { display: inline-block; padding: 10px 20px; }
> /* Links sit side by side with controllable size */
> ```

---

**Q4. Why should you always use `box-sizing: border-box`? Show the difference with an example.**

> Without `border-box`, padding and border are **added to** the specified width, making elements wider than expected:
>
> ```css
> /* Default: content-box */
...
> /* Actual width: 300px exactly (content adjusts to 250px) */
> ```
>
> This makes sizing predictable and prevents layout bugs. The universal reset should always include:
> ```css
...
**Q5. Write CSS for a card component that has: white background, 1px solid light gray border, 8px border-radius, 25px padding, 20px bottom margin, and a 4px blue left border accent. Also add a hover effect that changes the left border to purple.**
...
> This creates a clean card with a blue accent border on the left. On hover, the accent changes to purple, giving visual feedback to the user. The `border-radius` creates subtle rounded corners, and `padding` provides comfortable inner spacing.
