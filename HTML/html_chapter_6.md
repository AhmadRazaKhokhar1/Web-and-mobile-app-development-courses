...
**Q1.** Which attribute is REQUIRED on every `<img>` tag for accessibility?
...
**Q2.** What type of element is `<img>`?

- A) Block element with closing tag
- B) Inline void element ✅
- C) Semantic block element
- D) Inline element with closing tag

---

**Q3.** Which image format is best for photographs?
...
**Q4.** What does `../` mean in a file path?

- A) Go into a subfolder
- B) Go up one folder level ✅
- C) Stay in the current folder
- D) Go to the root of the website
...
**Q6.** Which tag provides a caption for an image?
...
**Q7.** What happens when an image fails to load?

- A) The page crashes
- B) Nothing appears
- C) The `alt` text is displayed instead ✅
- D) The `title` text is displayed

---

**Q8.** How do you make an image clickable (a link)?

- A) Add `href` to the `<img>` tag
- B) Wrap `<img>` inside an `<a>` tag ✅
- C) Use `<link>` tag with image
- D) Add `click="true"` to `<img>`

---

**Q9.** Which of the following is a BAD image file name?

- A) `hero-banner.jpg`
- B) `team-photo.png`
- C) `my vacation photo (1).jpg` ✅
- D) `product-headphones.webp`

---

**Q10.** When should you use `alt=""`(empty alt)?

- A) When you forget to write alt text
- B) For every image
- C) For purely decorative images that add no information ✅
- D) For linked images
...
**Q1. Why is the `alt` attribute so important? Give three reasons.**

> The `alt` attribute is critical for three reasons:
> 1. **Accessibility:** Screen readers read the `alt` text aloud to visually impaired users, allowing them to understand the image content
> 2. **Broken Images:** If an image fails to load (wrong path, server error, slow connection), the `alt` text is displayed in its place so users still get the information
> 3. **SEO:** Search engines cannot "see" images — they rely on `alt` text to understand what the image shows, which helps with image search rankings and overall page relevance

---

**Q2. Explain the difference between `<div>` and `<span>` with a practical example of each.**

> `<div>` is a **block-level** container that takes up the full width and starts on a new line. It's used to group large sections of content.
> ```html
> <div>
>     <h2>Our Services</h2>
...
> ```
>
> `<span>` is an **inline** container that stays within the text flow and only takes up the width of its content. It's used to target specific words or phrases.
> ```html
> <p>The price is <span>$49.99</span> per month.</p>
> ```
>
> Neither has any semantic meaning — they exist purely for CSS targeting and grouping.

---

**Q3. You have the following folder structure. Write the correct `<img>` tag to display `team.jpg` from the `about.html` page.**
...
> ```html
> <img src="../images/team.jpg" alt="Our team photo" width="600" height="400">
> ```
>
> Explanation: From `pages/about.html`, we need to go **up one level** (`../`) to reach `my-website/`, then go **into** the `images/` folder to find `team.jpg`.

---

**Q4. What is `<figure>` and `<figcaption>`? When would you use them instead of just `<img>` alone?**

> `<figure>` is a semantic container that groups an image (or other self-contained content) with its `<figcaption>` (caption).
>
> Use them when an image needs a **visible caption** that explains or credits the image:
> ```html
> <figure>
>     <img src="sunset.jpg" alt="Sunset at the beach" width="500" height="300">
>     <figcaption>Sunset at Clifton Beach, Karachi — Photo by Ali Khan</figcaption>
> </figure>
> ```
>
> Use `<img>` alone when the image doesn't need a visible caption (like icons, logos, or decorative images).

---

**Q5. What is "div soup" and why should you avoid it? How can you fix it?**

> "Div soup" refers to excessive, unnecessary nesting of `<div>` elements that makes code messy, hard to read, and difficult to maintain.
>
> ```html
> <!-- ❌ Div soup -->
> <div>
...
>
> **How to fix it:**
> 1. Only use `<div>` when you genuinely need to group content for CSS
> 2. Use **semantic tags** instead (`<header>`, `<nav>`, `<main>`, `<section>`, `<footer>`)
> 3. Remove any `<div>` that doesn't serve a clear grouping purpose
>
> ```html
> <!-- ✅ Clean -->
> <p>Simple content needs no extra divs.</p>
> ```
