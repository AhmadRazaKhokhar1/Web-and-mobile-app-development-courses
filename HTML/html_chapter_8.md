...
**Q1.** What should the homepage of a website always be named?
...
**Q2.** Which folder structure practice is CORRECT?

- A) All files in one folder
- B) Images in a separate `images/` folder ✅
- C) No folders needed for small projects
- D) CSS files mixed with HTML files

---

**Q3.** What is the difference between `class` and `id`?

- A) They are the same thing
- B) `class` is unique, `id` is reusable
- C) `class` is reusable, `id` is unique ✅
- D) Neither is used in HTML

---

**Q4.** Which file name follows best practices?
...
**Q5.** How do you link to a CSS file from HTML?
...
**Q6.** From `pages/about.html`, how do you link to `images/logo.png`?
...
- A) A recipe for web pages
- B) Using semantic tags correctly
- C) Excessive, unnecessary nesting of `<div>` elements ✅
- D) A CSS technique

---

**Q8.** Which class name follows best practices?

- A) `class="blueBox"`
- B) `class="div1"`
- C) `class="hero-section"` ✅
- D) `class="x"`

---

**Q9.** What tool should you use to check HTML for errors?

- A) Google Search
- B) W3C Markup Validator ✅
- C) Microsoft Word
- D) Photoshop

---

**Q10.** After mastering HTML, what should you learn next?
...
**Q1. Describe the ideal folder structure for a professional multi-page website with at least 5 pages, images, and preparation for CSS.**
...
> │       └── member-2.jpg
> ├── css/
> │   └── style.css           ← Ready for CSS
> └── js/
>     └── script.js           ← Ready for JavaScript
> ```
>
> Key principles: `index.html` at root, images in dedicated folder with subfolders, separate folders for blog, pages, CSS, and JS. All file names lowercase with hyphens.

---

**Q2. What is the difference between `class` and `id` attributes? When should you use each one?**

> **`class`** is a **reusable** attribute that can be applied to **multiple elements**. Use it to style groups of similar elements.
> ```html
...
> ```
>
> **`id`** is a **unique** attribute that can only be used on **ONE element** per page. Use it to target a single, specific element.
> ```html
> <section id="hero">This is the hero section</section>
> ```
>
> **Rule of thumb:** Use `class` for styling repeated patterns. Use `id` for unique page sections or JavaScript targets.

---

**Q3. List 5 common HTML mistakes and explain how to fix each one.**

> 1. **Missing `<!DOCTYPE html>`** → Always start every file with `<!DOCTYPE html>` to activate standards mode
>
> 2. **Using `<br>` for spacing** → Use separate `<p>` tags for paragraphs; control spacing with CSS margins
>
> 3. **Choosing headings for size** → Choose headings by meaning/importance (`h1` → `h2` → `h3`); use CSS for sizing
>
> 4. **Missing `alt` on images** → Always add descriptive `alt` text: `<img src="photo.jpg" alt="Team photo at office">`
>
> 5. **Using `<div>` instead of semantic tags** → Replace with `<header>`, `<nav>`, `<main>`, `<section>`, `<article>`, `<aside>`, `<footer>` where appropriate

---

**Q4. How does well-structured HTML make CSS easier? Give two specific examples.**

> Well-structured HTML gives CSS **clear targets** to style:
>
> **Example 1: Semantic tags as selectors**
> ```html
> <!-- HTML -->
...
> ```
> ```css
> /* CSS can directly target these meaningful tags */
...
> ```
>
> **Example 2: Classes for reusable components**
> ```html
> <!-- HTML -->
...
> ```
> ```css
> /* CSS styles ALL cards at once */
> .card { border: 1px solid gray; border-radius: 8px; padding: 20px; }
> ```
>
> Without proper HTML structure and classes, CSS would have nothing meaningful to target.

---

**Q5. You've completed the entire HTML course. List the correct learning path from here and explain why this order matters.**

> The correct learning path is:
>
> 1. **HTML** ✅ (Completed!) — Structure and content
> 2. **CSS** (Next) — Styling, layout, responsive design
> 3. **JavaScript** — Interactivity and dynamic behavior
> 4. **Git & GitHub** — Version control and collaboration
> 5. **Frameworks** (React, etc.) — Building complex applications
>
> **Why this order matters:**
> - CSS **styles HTML elements** — you need to understand HTML structure before you can style it
> - JavaScript **manipulates HTML elements** — you need to know what you're manipulating
> - Frameworks **generate HTML** — you need to understand what they're generating
>
> Skipping HTML or CSS and jumping to JavaScript or React is like trying to decorate a house that hasn't been built yet. The foundation (HTML) must come first.
