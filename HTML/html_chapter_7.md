...
**Q1.** Which semantic tag should wrap the primary, unique content of a page?
...
**Q3.** Which tag represents self-contained, independently shareable content?
...
**Q4.** What is the purpose of the `<label>` tag in forms?

- A) Creates a button
- B) Styles the input field
- C) Associates text with an input for accessibility ✅
- D) Validates form data

---

**Q5.** Which input type creates a dropdown calendar for selecting a date?
...
**Q6.** What attribute makes a form field mandatory?
...
**Q7.** Which table tag is used for header cells?
...
**Q8.** What is the `<aside>` tag used for?

- A) The main content of the page
- B) Navigation links
- C) Supplementary/sidebar content ✅
- D) The page footer

---

**Q9.** Which HTML attribute links a `<label>` to its `<input>`?

- A) `name`
- B) `class`
- C) `for` (matching input's `id`) ✅
- D) `href`

---

**Q10.** Should HTML tables be used for page layout?

- A) Yes, it's the standard method
- B) Yes, but only for simple layouts
- C) No, tables are for data only; use CSS for layout ✅
- D) No, tables are deprecated

---

## 📝 5 Short Answer Questions

**Q1. What is the difference between `<section>` and `<article>`? Give one example of each.**

> `<section>` groups **thematically related content** that is part of a larger whole. It doesn't make sense on its own.
> ```html
> <section>
>     <h2>Our Services</h2>
...
> ```
>
> `<article>` represents **self-contained, independent content** that could be shared or reused on its own (like in an RSS feed).
> ```html
> <article>
...
> ```
>
> **Test:** If the content could stand alone as a blog post, news item, or product card → use `<article>`. If it's a themed group within a page → use `<section>`.

---

**Q2. Why is the `<label>` tag important for form accessibility? How does the `for` attribute work?**

> The `<label>` tag is critical for accessibility because:
> 1. **Screen readers** read the label text when the input is focused, so visually impaired users know what to enter
> 2. **Clicking the label** focuses/activates the associated input, making it easier to use (especially for checkboxes and radio buttons on mobile)
>
> The `for` attribute works by matching the `id` of the input:
> ```html
> <label for="email">Email:</label>
> <input type="email" id="email" name="email">
> ```
> The value of `for="email"` must exactly match `id="email"` to link them together.

---

**Q3. Write a complete HTML table showing 3 products with columns for Product Name, Price, and In Stock (Yes/No). Include proper `<thead>`, `<tbody>`, and `<caption>`.**
...
**Q4. Explain the difference between `<header>`, `<main>`, and `<footer>`. How do they work together on a page?**

> - **`<header>`** contains introductory content — the site logo, title, tagline, and navigation. It appears at the **top** of the page and is typically the **same across all pages**.
>
> - **`<main>`** wraps the **primary, unique content** of the page — the content that **changes from page to page**. There can be only **ONE** `<main>` per page.
>
> - **`<footer>`** contains closing content — copyright notice, secondary navigation, contact info, social links. It appears at the **bottom** and is typically the **same across all pages**.
>
> Together they form the basic page structure:
> ```html
> <header>Same on all pages: Logo + Nav</header>
> <main>Unique content for THIS page</main>
> <footer>Same on all pages: Copyright + Links</footer>
> ```

---

**Q5. List 4 HTML5 form validation attributes and explain what each one does with an example.**

> 1. **`required`** — Field must be filled before form submission
>    ```html
>    <input type="text" name="name" required>
>    ```
>
> 2. **`minlength`** — Minimum number of characters required
>    ```html
>    <input type="text" name="username" minlength="3">
>    ```
>
> 3. **`max`** — Maximum numeric value allowed
>    ```html
>    <input type="number" name="age" max="120">
>    ```
>
> 4. **`pattern`** — Must match a specific regex pattern
>    ```html
>    <input type="text" name="zip" pattern="[0-9]{5}" title="Enter 5-digit zip code">
>    ```
>
> These attributes enable **client-side validation** without needing any JavaScript — the browser checks the rules automatically when the user submits the form.
