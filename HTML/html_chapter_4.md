...
**Q1.** Which of the following is an INLINE element?
...
**Q2.** What is the key difference between `<strong>` and `<b>`?

- A) `<strong>` is italic, `<b>` is bold
- B) `<strong>` carries semantic importance, `<b>` is just visual ✅
- C) They are exactly the same
- D) `<b>` is semantic, `<strong>` is not

---

**Q3.** Which tag would you use to show a chemical formula like H₂O?
...
**Q4.** What does the `<mark>` tag do?

- A) Makes text bold
- B) Makes text italic
- C) Highlights text with a yellow background ✅
- D) Deletes text

---

**Q5.** Which of the following is CORRECT nesting?

- A) `<strong><p>Text</p></strong>`
- B) `<p><strong>Text</strong></p>` ✅
- C) `<em><h2>Text</h2></em>`
- D) `<span><div>Text</div></span>`

---

**Q6.** What does `<del>` represent?

- A) Delivered text
- B) Deleted/removed text ✅
- C) Delayed text
- D) Decorated text

---

**Q7.** Which tag should be used for copyright fine print at the bottom of a page?
...
**Q8.** How do block elements stack on a page?

- A) Horizontally (left to right)
- B) Vertically (top to bottom) ✅
- C) Diagonally
- D) They overlap each other

---

**Q9.** What is the correct way to write E = mc² in HTML?

- A) `E = mc<sub>2</sub>`
- B) `E = mc<sup>2</sup>` ✅
- C) `E = mc<small>2</small>`
- D) `E = mc<mark>2</mark>`

---

**Q10.** Which tag shows the full form of an abbreviation when hovered?
...
**Q1. Explain the difference between block-level and inline elements with one example of each.**

> **Block elements** start on a new line and take up the full available width. Example: `<p>This is a paragraph.</p>` — it occupies the entire row.
>
> **Inline elements** do NOT start on a new line — they flow within text and only take up as much space as their content. Example: `<strong>important</strong>` — it sits within a line of text.
>
> ```html
> <p>This is a <strong>block containing inline</strong> text.</p>
> ```

---

**Q2. Why should you use `<strong>` instead of `<b>` in most cases?**

> `<strong>` is a **semantic** tag that tells browsers, screen readers, and search engines that the text is **genuinely important**. A screen reader will change its tone to convey importance. Google considers it as significant content.
>
> `<b>` is **non-semantic** — it only makes text visually bold without any meaning. A screen reader reads it normally, and Google gives it no special importance.
>
> Using `<strong>` improves **accessibility, SEO, and code meaning**.

---

**Q3. Write HTML code that shows: "The price was ~~$99.99~~ and is now $59.99 (limited offer)" using appropriate semantic tags.**

```html
<p>The price was <del>$99.99</del> and is now 
<strong>$59.99</strong> <small>(limited offer)</small></p>
```

> - `<del>` for the old deleted price
> - `<strong>` for the new important price
> - `<small>` for the fine-print note

---

**Q4. What is the difference between `<em>` and `<i>`? Give one appropriate use case for each.**

> `<em>` is **semantic** — it means the text is **stressed/emphasized**, and it changes the meaning of the sentence. Screen readers will vocally emphasize it.
> - **Use case:** `<p>I said <em>today</em>, not tomorrow.</p>` (stressing the word "today")
>
> `<i>` is **non-semantic** — it just makes text italic visually without adding emphasis.
> - **Use case:** `<p>The French word <i>bonjour</i> means hello.</p>` (foreign word, no emphasis intended)

---

**Q5. Why is it wrong to put a block element inside an inline element? Give an incorrect and corrected example.**

> Block elements define large structural sections, while inline elements live within text flow. Putting a block inside an inline breaks the document structure and can cause unpredictable rendering.
>
> ```html
> <!-- ❌ WRONG: Block (<p>) inside inline (<strong>) -->
> <strong><p>This is wrong</p></strong>
>
> <!-- ✅ CORRECT: Inline (<strong>) inside block (<p>) -->
> <p><strong>This is correct</strong></p>
> ```
>
> **Rule:** Inline elements go INSIDE block elements, never the reverse.
