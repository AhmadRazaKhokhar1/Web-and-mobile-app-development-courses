...
**Q1.** What must be the very first line in an HTML5 document?
...
**Q2.** What does the `lang="en"` attribute do in `<html lang="en">`?

- A) Changes the page color to English theme
- B) Specifies the language of the page content ✅
- C) Translates the page automatically
- D) Sets the font to English

---

**Q3.** Which section contains metadata that is NOT visible on the page?
...
**Q4.** Where does the `<title>` text appear?

- A) As a heading on the page
- B) In the browser tab ✅
- C) In the footer
- D) As a popup message

---

**Q5.** What character encoding should you always use?
...
**Q6.** What is the correct syntax for an HTML comment?
...
**Q7.** How many `<body>` tags can an HTML page have?
...
**Q8.** What is wrong with this code: `<p><strong>Hello</p></strong>`?

- A) Nothing is wrong
- B) Incorrect nesting order ✅
- C) Missing DOCTYPE
- D) `<strong>` is not a real tag

---

**Q9.** What does the viewport meta tag control?

- A) Page color scheme
- B) Page scaling on mobile devices ✅
- C) Browser language
- D) Page loading speed

---

**Q10.** In VS Code, what shortcut generates the HTML5 boilerplate?

- A) Type `html` and press Enter
- B) Type `#` and press Tab
- C) Type `!` and press Tab ✅
- D) Type `boilerplate` and press Enter

---

## 📝 5 Short Answer Questions

**Q1. What is the purpose of `<!DOCTYPE html>` and what happens if you skip it?**

> `<!DOCTYPE html>` tells the browser that the document is written in HTML5 and should be rendered using modern standards. If you skip it, the browser enters **Quirks Mode**, where it guesses how to render the page. This can cause inconsistent layouts and unexpected behavior across different browsers.

---

**Q2. What is the difference between `<head>` and `<body>`?**

> The `<head>` section contains **metadata** — information about the page that is NOT displayed on the screen (like the title, character encoding, and description). The `<body>` section contains all the **visible content** that users see and interact with (headings, paragraphs, images, links, etc.).

---

**Q3. Why is the `lang` attribute important on the `<html>` tag? Give two reasons.**

> 1. **Accessibility:** Screen readers use the `lang` attribute to pronounce words in the correct language and accent, helping visually impaired users.
> 2. **SEO:** Search engines like Google use the `lang` attribute to understand the content language and show the page to the right audience in search results.

---

**Q4. Write the correct nesting for a paragraph containing bold and italic text that says "HTML is amazing".**

```html
<p><strong><em>HTML is amazing</em></strong></p>
```

> The rule is **last opened = first closed**. `<em>` was opened last, so it closes first. Then `<strong>` closes. Finally `<p>` closes.

---

**Q5. List all the essential tags/elements that every proper HTML5 page must have.**

> Every proper HTML5 page must have:
...
> 5. `<meta name="viewport">` — Mobile responsiveness
> 6. `<title>` — Browser tab title
> 7. `<body>` — Visible content container
